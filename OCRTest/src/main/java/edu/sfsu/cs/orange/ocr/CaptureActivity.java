...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback, 
  ...
  public void onCreate(Bundle icicle) {
    ...
    cameraManager.setFinderRect(viewfinderView.getFinderRect());
...
//        }
  ...
  boolean handleOcrDecode(OcrResult ocrResult) {
    ...
    if (lastResult != null) {
      int confidence = 0;
      String[] strings = lastResult.getText().split(" |\n");
      if(strings.length != lastResult.getWordConfidences().length){
        return false;
      }
      int i = 0;
      String target = null;
      for (int wordConfidence : lastResult.getWordConfidences()) {
      ...
      if (target != null) {
        final TextView ResTextView = (TextView) findViewById(R.id.single_result_view);
        ResTextView.setText(target);
      }
    }
...
//    }
  ...
  void handleOcrContinuousDecode(OcrResult ocrResult) {
    ...
    lastResult = ocrResult;
    List<Rect> characterBoundingBoxes = lastResult.getWordBoundingBoxes();

    String[] strings = lastResult.getText().split(" |\n");
    int centerItem = getCenterItem(characterBoundingBoxes);
    String target = strings[centerItem];
    Log.d(TAG, "handleOcrContinuousDecode: " + target);
    if (target != null) {
      final TextView ResTextView = (TextView) findViewById(R.id.single_result_view);
      ResTextView.setText(target);
    }
...
//    }
   ...
   * 计算中间的ITEM
   * @return
   * @param characterBoundingBoxes
   */
    int getCenterItem(List<Rect> characterBoundingBoxes){
      Rect finderRect = cameraManager.getFramingRectInPreview();
      double minDistance = finderRect.width();
      int i = 0;
      int tempIndex = 0;
      for (Rect characterBoundingBox : characterBoundingBoxes) {
        double powX = Math.pow(finderRect.centerX()-finderRect.left - characterBoundingBox.centerX(), 2);
        double powY = Math.pow(finderRect.centerY()-finderRect.top - characterBoundingBox.centerY(), 2);
        double distance = Math.sqrt(powX + powY);
        Log.d(TAG, "handleOcrContinuousDecode: " + distance);
